...
import com.sticker.production_service.client.OrderClient;
...
public class OrdenProduccionService {
    ...
    public OrdenProduccion crear(OrdenProduccionDTO dto) {
        ...
        // Valida que el pedido existe en order-service
        try {
            Map<String, Object> pedido = orderClient.obtenerPedido(dto.getPedidoId());
            if (pedido == null) {
                throw new RuntimeException("Pedido no encontrado");
            }
            log.info("Pedido validado con estado: {}", pedido.get("estadoPago"));
        } catch (Exception e) {
            log.error("Error al validar pedido: {}", e.getMessage());
            throw new RuntimeException("No se pudo validar el pedido: " + dto.getPedidoId());
        }
        ...
        log.info("Orden de produccion creada con id: {}", guardada.getId());
        return guardada;
    }
