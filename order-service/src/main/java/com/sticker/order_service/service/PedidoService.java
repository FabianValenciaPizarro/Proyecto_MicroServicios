...
import com.sticker.order_service.client.UserClient;
...
public class PedidoService {
    ...
    public Pedido crear(PedidoDTO dto) {
        ...
        // Valida que el usuario existe en user-service
        try {
            Map<String, Object> usuario = userClient.obtenerUsuario(dto.getUsuarioId());
            if (usuario == null) {
                throw new RuntimeException("Usuario no encontrado");
            }
            log.info("Usuario validado: {}", usuario.get("username"));
        } catch (Exception e) {
            log.error("Error al validar usuario: {}", e.getMessage());
            throw new RuntimeException("No se pudo validar el usuario: " + dto.getUsuarioId());
        }

        // Consulta precio del material en catalog-service
